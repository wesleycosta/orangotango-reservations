...
internal sealed class CheckInCommandHandler(IUnitOfWork unitOfWork,
    IValidator<CheckInCommand> validator,
    IReservationMapper _mapper,
    IReservationRepository _repository,
    IReservationPublisher _publisher) : CommandHandlerBase<CheckInCommand>(unitOfWork, validator)
{
    public override async Task<Result> Handle(CheckInCommand command, CancellationToken cancellationToken)
    {
        if (!await Validate(command))
            return BadResult();

        var reservation = await _repository.GetById(command.Id);
        reservation.ExecuteCheckIn();
